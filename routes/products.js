...
router.post(`/`, async (req, res) => {
    ...
    if (!product) return res.status(500).send('The product can not create')
...
router.put('/:id', async (req, res) => {
    ...
    if (!product) return res.status(404).send('The product can not be update');
    res.send(product);
})
...
router.delete('/:id', (req, res) => {
    Product.findByIdAndRemove(req.params.id).then(product => {
        if (product) {
            return res.status(200).json({success: true, message: 'The product is remove'})
        } else {
